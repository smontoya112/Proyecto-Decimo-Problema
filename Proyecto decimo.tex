...
de $m$ bits requieren tiempo $\tilde{O}(m)$. Esto se debe a que existen algoritmos
...
$m$ bits, las mismas operaciones toman tiempo $\tilde{O}(d \cdot m)$, pues ahora debemos
...
$\text{mcd}$ toma tiempo $O(\log n)$ usando el Algoritmo de Euclides, y por lo tanto la complejidad temporal de este
...
probabilísticos, como el test de Miller-Rabin. Estos algoritmos son muy eficientes,
...
original es $\tilde{O}(\log^{21/2} n)$, y aunque versiones mejoradas la reducen a
$\tilde{O}(\log^6 n)$, esto sigue siendo considerablemente más lento que los
algoritmos probabilísticos usados en la práctica, como Miller-Rabin, cuya complejidad
...
lo cual es completamente inviable en la práctica.
...
más favorable entre velocidad y confiabilidad. La probabilidad de error del test
...
generalizada, requerían asumir conjeturas no demostradas para garantizar su
